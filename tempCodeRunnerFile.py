"""
Traffic Violations Explainable System — Tkinter GUI  (v2)
Improvements:
  • Equal-height violation cards + output box (vertical PanedWindow splitter)
  • Resizable left panel (horizontal PanedWindow splitter)
  • Manual Prolog-style fact query input box
  • Correct "unknown vehicle / missing data" → INSUFFICIENT DATA (not false violation)
  • Download-as-PDF button (reportlab)
"""
...
VIOL  = "violation"
CLEAR = "clear"
INSUF = "insufficient"   # data needed for this check was not supplied


# ── violation engine (with proper INSUF handling) ─────────────────────────────
...
def simulate_violations(facts: list) -> dict:
    vehicles  = {}
    tl_red    = None   # None means not asserted
    crossed   = set()
    speeds    = {}
    wrong_ln  = set()
    helmets   = {}
...
class TrafficApp(tk.Tk):
    ...
    def _build_form(self, outer):
        ...
        tk.Label(form,
                 text="Type Prolog-style facts to add/override form data.\n"
                      "Example:\n"
                      "  vehicle(truck_99, truck). speed(truck_99, 110).\n"
                      "  traffic_light(red, t_001). phone_usage(truck_99, yes).",
                 font=("Courier New", 7),
        ...
        tk.Checkbutton(form, text="Use manual query (ignore form fields above)",
    ...
    def _build_results(self, parent):
        ...
        pdf_state = tk.NORMAL if REPORTLAB else tk.DISABLED
        ...
        # Vertical PanedWindow — cards top, output bottom, EQUAL sizes
        ...
        # equalise pane heights after first render
    ...
    def _on_check(self):
        ...
        if use_manual:
            if not manual_txt:
                messagebox.showerror("Empty Query",
                                     "Please type facts in the Manual Fact Query box,\n"
                                     "or uncheck 'Use manual query'.")
                return
            facts = parse_manual_query(manual_txt)
            vids  = [args[0] for name, args in facts if name == "vehicle"]
            if not vids:
                messagebox.showerror("No vehicle/2 fact",
                                     "Add at least one vehicle fact.\n"
                                     "Example: vehicle(v1, car).")
                return
            vid = vids[0]
        else:
    ...
    def _make_card(self, parent, vtype: str, status: str, explanation: str):
        color = DANGER if status == VIOL else (WARN if status == INSUF else SUCCESS)
        icon  = "🔴" if status == VIOL else ("❓" if status == INSUF else "🟢")
        badge = "✔ HIT" if status == VIOL else ("? DATA" if status == INSUF else "✘ OK")
    ...
    # ── clear ─────────────────────────────────────────────────────────────────
    def _on_clear(self):
        self._v_id.set(""); self._v_type.set("car"); self._v_speed.set("e.g.  85")
        self._v_tl_red.set(False); self._v_crossed.set(False); self._v_wrong.set(False)
        self._v_near_sb.set(False); self._v_dir.set("none"); self._v_emerg.set(False)
        self._v_helmet.set(""); self._v_belt.set(""); self._v_phone.set("")
        self._v_pax.set("e.g.  3"); self._v_parked.set(False); self._v_nopk.set(False)
        self._v_manual.delete("1.0", tk.END); self._v_use_manual.set(False)
        for w in self._grid.winfo_children(): w.destroy()
        self._out.config(state=tk.NORMAL); self._out.delete("1.0", tk.END)
        self._out.config(state=tk.DISABLED)
        self._v_title.config(text="Enter vehicle details and press CHECK VIOLATIONS →")
        self._v_badge.config(text=""); self._pdf_btn.config(state=tk.DISABLED)
        self._current_vid = ""; self._report_text = ""
